...
def compare_positions(benchmark_video, user_video):
    ...
    webcam_fps = user_cam.get(cv2.CAP_PROP_FPS)
    print(webcam_fps)
    fps_time = 0  # Initializing fps to 0

    detector_1 = pm.poseDetector(False, False, True, 0.85, 0.85)
    detector_2 = pm.poseDetector(False, False, True, 0.85, 0.85)
    frame_counter = 0
    correct_frames = 0
    ...
    while (benchmark_cam.isOpened() or user_cam.isOpened()):
        try:
            ...
            if ret_val_1 or ret_val:
                ...
                if error < 0.1:
                    cv2.putText(image_1, "CORRECT POSE", (40, 600),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    correct_frames += 1
                else:
                    cv2.putText(image_1,  "INCORRECT POSE", (40, 600),
                ...
                cv2.putText(image_1, "Yoga Flow Accurately Done: {}%".format(str(round(100*correct_frames/frame_counter, 2))), (10, 70),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)

                # Display both the benchmark and the user videos
                cv2.imshow('Reference Video', image_2)
